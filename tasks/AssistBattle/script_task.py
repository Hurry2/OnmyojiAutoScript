...
class ScriptTask(
    ...
    def run(self):
        self.conf = self.config.AssistBattle.assist_battle_config
        ...
        if self.conf.evozone_enable and evozone_res > 0:
            self.run_evozone(evozone_res)
            self.goto_page(page_main)
        if self.conf.realmraid_enable and realmraid_res > 0:
    ...
    def run_evozone(self, count: int):
        ...
        self.config.evozone.evo_zone_config.layer = Layer.ONE
        self.config.evozone.evo_zone_config.kirin_type = KirinType.LIGHTNINGKIRIN
    ...
    def run_realmraid(self, count: int):
        ...
        self.config.realmraid.realm_raid_config.number_attack = count
        self.config.realmraid.realm_raid_config.exit_four = False
        self.config.realmraid.realm_raid_config.order_attack = '0 > 1 > 2 > 3 > 4 > 5'
        self.goto_page(page_realm_raid)
        if self.conf.switch_soul_enable:
            self.config.realmraid.realm_raid_config.switch_soul_config.enable = True
        RealmRaidScriptTask.run()
