...
class ScriptTask(GameUi, GeneralBattle, SwitchSoul, DuelAssets, SwitchOnmyoji):
    ...
    def can_start_duel(self) -> bool:
        ...
        if not self.conf.duel_celeb_config.celeb_battle and self.is_celeb:
            ...
            return False
        # 练习
        if self.appear(self.I_BATTLE_WITH_TRAIN) or self.appear(
            self.I_BATTLE_WITH_TRAIN2
        ):
            return False
        # 荣誉满了，退出
    ...
    def start_duel(self):
        ...
        task_run_time_seconds = timedelta(
            seconds=int((datetime.now() - self.start_time).total_seconds())
        )
        logger.info(f'battle result: {battle_ret}')
        logger.info(
            f'battle count:{self.current_count} | win:{self.battle_win_count} failure:{self.battle_lose_count}'
        )
    ...
    def battle_prepare(self):
        ...
        while True:
            if (
                not_in_prepare_cnt >= max_retry
            ):  # max_retry次识别不到任何阶段元素(准备,战斗,结算), 退出
                break
            self.screenshot()
            if (
                self.is_battle_end() or self.is_in_real_battle()
            ):  # 战斗已经结束或已经开始战斗
                break
            if (
                not self.is_in_battle_prepare()
            ):  # 一般不会出现这种情况(不在准备,战斗,结束界面), 但是处理一下
            ...
            if self.appear_then_click(
                self.I_D_AUTO_ENTRY, interval=1.2
    ...
    def wait_battle(self) -> bool:
        ...
        while True:
            self.screenshot()
            self.check_and_get_reward()
            if self.appear(self.I_CHECK_DUEL) and self.appear(
                self.I_D_HELP
            ):  # 斗技主界面
                break
            if self.appear(self.I_D_WIN_SHARE, interval=1.2):  # 拔得头筹
                ...
                continue
            if self.appear_then_click(
                self.I_UI_BACK_RED, interval=1.2
            ):  # 关闭段位上升页面
                ret_timer.reset()
                continue
            if (
                ret_timer.started() and ret_timer.reached()
            ):  # 兜底逻辑, 已经结算了但是还没有到斗技主界面
                self.goto_page(page_duel)
                break
    ...
    def duel_exit_battle(self):
        while 1:
            ...
            # 选式神界面退出或战斗内退出
            if self.appear_then_click(
                self.I_DUEL_EXIT, interval=1
            ) or self.appear_then_click(self.I_EXIT, interval=1):
                continue
    ...
    def switch_soul(self):
        ...
        if self.conf.switch_soul.enable_switch_by_name:
            ...
            self.run_switch_soul_by_name(
                self.conf.switch_soul.group_name, self.conf.switch_soul.team_name
            )
    ...
    def duel_main(self, screenshot=False) -> bool:
        ...
        return (
            ...
            or self.appear(self.I_CHECK_DUEL)
    ...
    def is_in_battle_prepare(self, skip_screenshot=True) -> bool:
        ...
        self.maybe_screenshot(skip_screenshot)
        return (
            self.appear(self.I_D_PREPARE)
            or self.appear(self.I_D_AUTO_ENTRY)
            or self.appear(self.I_BAN)
            or self.appear(self.I_D_WORD_BATTLE)
            or self.appear(self.I_D_CHECK_BAN)
        )
    ...
    def is_battle_end(self) -> bool:
        return (
            self.is_battle_win()
            or self.is_battle_lose()
            or self.appear(self.I_REWARD)
            or self.appear(self.I_UI_REWARD)
        )
