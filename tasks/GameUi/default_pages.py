...
def random_click(
    ...
    Args:
        ...
        delay: 手动指定的延迟时间,范围为其0.1的正态分布。

    Returns:
        单个 `RuleClick`，或一个由多个 `RuleClick` 组成的列表。
    """
    if delay is not None:
        random_sleep = random.gauss(delay, 0.1)
    ...
    click = random.choice(
        [item for item, enabled in zip(click_area_list, ltrb) if enabled]
    )
...
page_shikigami_records.add_enter_failure_hooks(
    conditional_action(
...
page_shikigami_records.connect(
    ...
    key="page_shikigami_records->page_main",
)
page_main.connect(
    page_shikigami_records,
    GameUiAssets.I_MAIN_GOTO_SHIKIGAMI_RECORDS,
    key="page_main->page_shikigami_records",
)

page_onmyodo = Page(GameUiAssets.I_CHECK_ONMYODO, category="global")
page_onmyodo.add_enter_failure_hooks(
    conditional_action(
        condition=GameUiAssets.I_CHECK_MAIN,
        action=RestartAssets.C_LOGIN_SCROLL_CLOSE_AREA,
    )
)
page_onmyodo.connect(
    page_main, GlobalGameAssets.I_UI_BACK_YELLOW, key="page_onmyodo->page_main"
)
page_main.connect(
    page_onmyodo, GameUiAssets.I_MAIN_GOTO_ONMYODO, key="page_main->page_onmyodo"
)

page_friends = Page(GameUiAssets.I_CHECK_FRIENDS, category="global")
page_friends.add_enter_failure_hooks(
    conditional_action(
        condition=GameUiAssets.I_CHECK_MAIN,
        action=RestartAssets.C_LOGIN_SCROLL_CLOSE_AREA,
    )
...
page_friends.connect(
    page_main, GlobalGameAssets.I_UI_BACK_RED, key="page_friends->page_main"
)
...
page_daily.add_enter_failure_hooks(
    ...
    conditional_action(
        ...
        action=lambda task: task.ui_click(
            click=random_click(ltrb=(False, False, False, True)),
            stop=GlobalGameAssets.I_UI_BACK_YELLOW,
        ),
    ),
)
page_daily.connect(
    page_main, GlobalGameAssets.I_UI_BACK_YELLOW, key="page_daily->page_main"
)
page_main.connect(
    page_daily, GameUiAssets.I_MAIN_GOTO_DAILY, key="page_main->page_daily"
)

page_courtyard_affairs = Page(
    DailyTriflesAssets.I_CHECK_COURTYARD_AFFAIRS, category="global"
)
page_main.connect(
    page_courtyard_affairs,
    DailyTriflesAssets.I_ENTER_COURTYARD_AFFAIRS,
    key="page_main->page_courtyard_affairs",
)
page_courtyard_affairs.connect(
    page_main,
    GlobalGameAssets.I_UI_BACK_YELLOW,
    key="page_courtyard_affairs->page_main",
)
page_courtyard_affairs.add_leave_failure_hooks(
...
page_mall.add_enter_failure_hooks(
    conditional_action(
        condition=GameUiAssets.I_CHECK_MAIN,
        action=RestartAssets.C_LOGIN_SCROLL_CLOSE_AREA,
    )
)
page_mall.add_enter_success_hooks(
    GameUiAssets.I_AD_CLOSE_RED,
    GlobalGameAssets.I_UI_BACK_RED,
    GlobalGameAssets.I_UI_CANCEL_SAMLL,
)
page_mall.connect(
    page_main, GlobalGameAssets.I_UI_BACK_YELLOW, key="page_mall->page_main"
)

page_mall_recommend = Page(GameUiAssets.I_CHECK_MALL_RECOMMEND, category="global")
page_mall_recommend.add_enter_success_hooks(
    GameUiAssets.I_AD_CLOSE_RED,
    GlobalGameAssets.I_UI_BACK_RED,
    GlobalGameAssets.I_UI_CANCEL_SAMLL,
)
page_mall_recommend.connect(
    page_mall, GlobalGameAssets.I_UI_BACK_YELLOW, key="page_mall_recommend->page_mall"
)
page_main.connect(
    page_mall_recommend,
    GameUiAssets.I_MAIN_GOTO_MALL,
    key="page_main->page_mall_recommend",
)
...
page_guild.add_enter_failure_hooks(
    conditional_action(
        condition=GameUiAssets.I_CHECK_MAIN,
        action=RestartAssets.C_LOGIN_SCROLL_CLOSE_AREA,
    )
...
page_guild.connect(
    page_main, GlobalGameAssets.I_UI_BACK_YELLOW, key="page_guild->page_main"
)
page_main.connect(
    page_guild, GameUiAssets.I_MAIN_GOTO_GUILD, key="page_main->page_guild"
)

page_shirin = Page(GameUiAssets.I_CHECK_SHRIN, category="global")
page_guild.connect(
    page_shirin, GameUiAssets.I_GUILD_TO_SHRIN, key="page_guild->page_shirin"
)
page_shirin.connect(
...
page_team.connect(
    page_main, GlobalGameAssets.I_UI_BACK_YELLOW, key="page_team->page_main"
)
...
page_collection.add_enter_failure_hooks(
    conditional_action(
        condition=GameUiAssets.I_CHECK_MAIN,
        action=RestartAssets.C_LOGIN_SCROLL_CLOSE_AREA,
    )
)
page_collection.connect(
    page_main, GlobalGameAssets.I_UI_BACK_YELLOW, key="page_collection->page_main"
)
page_main.connect(
    page_collection,
    GameUiAssets.I_MAIN_GOTO_COLLECTION,
    key="page_main->page_collection",
)

page_travel = Page(GameUiAssets.I_CHECK_TRAVEL, category="global")
page_travel.add_enter_failure_hooks(
    conditional_action(
        condition=GameUiAssets.I_CHECK_MAIN,
        action=RestartAssets.C_LOGIN_SCROLL_CLOSE_AREA,
    )
...
page_travel.connect(
    page_main, GlobalGameAssets.I_UI_BACK_YELLOW, key="page_travel->page_main"
)
page_main.connect(
    page_travel, GameUiAssets.I_MAIN_GOTO_TRAVEL, key="page_main->page_travel"
)

page_pet = Page(
    any_of(PetsAssets.I_PET_CLAW, PetsAssets.I_PET_FEAST), category="global"
)
page_pet.connect(
    page_main, GlobalGameAssets.I_UI_BACK_CIRCLE, key="page_pet->page_main"
)
page_main.connect(page_pet, PetsAssets.I_PET_HOUSE, key="page_main->page_pet")
page_pet.add_enter_success_hooks(
    conditional_action(
        condition=lambda task: not task.appear(PetsAssets.I_PET_FEAST),
...
page_main.connect(
    page_act_list, GameUiAssets.I_ACT_LIST_EXPAND, key="page_main->page_act_list"
)
page_act_list.connect(
    page_main, GlobalGameAssets.I_UI_BACK_YELLOW, key="page_act_list->page_main"
)

# 召唤页。
page_summon = Page(GameUiAssets.I_CHECK_SUMMON, category="global")
page_summon.connect(
    page_main, GlobalGameAssets.I_UI_BACK_YELLOW, key="page_summon->page_main"
)
page_main.connect(
...
page_duel.connect(
    page_town, GlobalGameAssets.I_UI_BACK_YELLOW, key="page_duel->page_town"
)
...
page_demon_encounter.connect(
    page_town, GlobalGameAssets.I_UI_BACK_YELLOW, key="page_demon_encounter->page_town"
)
page_town.connect(
    page_demon_encounter,
    GameUiAssets.I_TOWN_GOTO_DEMON_ENCOUNTER,
    key="page_town->page_demon_encounter",
)

page_hunt = Page(GameUiAssets.I_CHECK_HUNT, category="global")
page_hunt.connect(
    page_town, GlobalGameAssets.I_UI_BACK_YELLOW, key="page_hunt->page_town"
)
...
page_hunt_kirin.connect(
    page_town, GlobalGameAssets.I_UI_BACK_YELLOW, key="page_hunt_kirin->page_town"
)
page_town.connect(
    page_hunt_kirin, GameUiAssets.I_TOWN_GOTO_HUNT, key="page_town->page_hunt_kirin"
)

page_draft_duel = Page(GameUiAssets.I_CHECK_DRAFT_DUEL, category="global")
page_draft_duel.connect(
    page_town, GlobalGameAssets.I_UI_BACK_YELLOW, key="page_draft_duel->page_town"
)
page_town.connect(
    page_draft_duel,
    GameUiAssets.I_TOWN_GOTO_DRAFT_DUEL,
    key="page_town->page_draft_duel",
)

page_hyakkiyakou = Page(GameUiAssets.I_CHECK_KYAKKIYAKOU, category="global")
page_hyakkiyakou.connect(
    page_town, GlobalGameAssets.I_UI_BACK_RED, key="page_hyakkiyakou->page_town"
)
page_town.connect(
    page_hyakkiyakou,
    GameUiAssets.I_TOWN_GOTO_HYAKKIYAKOU,
    key="page_town->page_hyakkiyakou",
)
...
page_exploration.connect(
    page_main, GlobalGameAssets.I_UI_BACK_YELLOW, key="page_exploration->page_main"
)
page_exploration.add_enter_failure_hooks(ExplorationAssets.I_E_OPEN_FOLDER)
page_main.connect(
    page_exploration,
    GameUiAssets.I_MAIN_GOTO_EXPLORATION,
    key="page_main->page_exploration",
)

# 探索区域页面。
page_awake_zones = Page(GameUiAssets.I_CHECK_AWAKE, category="global")
page_awake_zones.connect(
    page_exploration,
    GlobalGameAssets.I_UI_BACK_YELLOW,
    key="page_awake_zones->page_exploration",
)
page_exploration.connect(
    page_awake_zones,
    GameUiAssets.I_EXPLORATION_GOTO_AWAKE_ZONE,
    key="page_exploration->page_awake_zones",
)

page_soul_zones = Page(GameUiAssets.I_CHECK_SOUL_ZONES, category="global")
page_soul_zones.connect(
    page_exploration,
    GlobalGameAssets.I_UI_BACK_YELLOW,
    key="page_soul_zones->page_exploration",
)
page_exploration.connect(
    page_soul_zones,
    GameUiAssets.I_EXPLORATION_GOTO_SOUL_ZONE,
    key="page_exploration->page_soul_zones",
)

page_realm_raid = Page(GameUiAssets.I_CHECK_REALM_RAID, category="global")
page_realm_raid.connect(
    page_exploration,
    GlobalGameAssets.I_UI_BACK_RED,
    key="page_realm_raid->page_exploration",
)
page_realm_raid.connect(
    page_shikigami_records,
    GameUiAssets.I_REALM_RAID_GOTO_SHIKIGAMI_RECORDS,
    key="page_realm_raid->page_shikigami_records",
)
page_exploration.connect(
    page_realm_raid,
    GameUiAssets.I_EXPLORATION_GOTO_REALM_RAID,
    key="page_exploration->page_realm_raid",
)

page_kekkai_toppa = Page(GameUiAssets.I_KEKKAI_TOPPA, category="global")
page_kekkai_toppa.connect(
    page_exploration,
    GlobalGameAssets.I_UI_BACK_RED,
    key="page_kekkai_toppa->page_exploration",
)
page_kekkai_toppa.connect(
    page_shikigami_records,
    GameUiAssets.I_REALM_RAID_GOTO_SHIKIGAMI_RECORDS,
    key="page_kekkai_toppa->page_shikigami_records",
)
page_realm_raid.connect(
    page_kekkai_toppa,
    RyouToppaAssets.I_RYOU_TOPPA,
    key="page_realm_raid->page_kekkai_toppa",
)
page_kekkai_toppa.connect(
    page_realm_raid,
    GameUiAssets.I_RYOUTOPPA_GOTO_REALMRAID,
    key="page_kekkai_toppa->page_realm_raid",
)

page_goryou_realm = Page(GameUiAssets.I_CHECK_GORYOU, category="global")
page_goryou_realm.connect(
    page_exploration,
    GlobalGameAssets.I_UI_BACK_YELLOW,
    key="page_goryou_realm->page_exploration",
)
page_exploration.connect(
    page_goryou_realm,
    GameUiAssets.I_EXPLORATION_GOTO_GORYOU_REALM,
    key="page_exploration->page_goryou_realm",
)

page_delegation = Page(GameUiAssets.I_CHECK_DELEGATION, category="global")
page_delegation.connect(
    page_exploration,
    GlobalGameAssets.I_UI_BACK_YELLOW,
    key="page_delegation->page_exploration",
)
page_exploration.connect(
    page_delegation,
    GameUiAssets.I_EXPLORATION_GOTO_DELEGATION,
    key="page_exploration->page_delegation",
)

page_secret_zones = Page(GameUiAssets.I_CHECK_SECRET_ZONES, category="global")
page_secret_zones.connect(
    page_exploration,
    GlobalGameAssets.I_UI_BACK_YELLOW,
    key="page_secret_zones->page_exploration",
)
page_exploration.connect(
    page_secret_zones,
    GameUiAssets.I_EXPLORATION_GOTO_SECRET_ZONES,
    key="page_exploration->page_secret_zones",
)

page_area_boss = Page(GameUiAssets.I_CHECK_AREA_BOSS, category="global")
page_area_boss.connect(
    page_exploration,
    GlobalGameAssets.I_UI_BACK_YELLOW,
    key="page_area_boss->page_exploration",
)
page_exploration.connect(
    page_area_boss,
    GameUiAssets.I_EXPLORATION_GOTO_AREA_BOSS,
    key="page_exploration->page_area_boss",
)

page_heian_kitan = Page(GameUiAssets.I_CHECK_HEIAN_KITAN, category="global")
page_heian_kitan.connect(
    page_exploration,
    GameUiAssets.I_CHECK_HEIAN_KITAN,
    key="page_heian_kitan->page_exploration",
)
page_exploration.connect(
    page_heian_kitan,
    GameUiAssets.I_EXPLORATION_GOTO_HEIAN_KITAN,
    key="page_exploration->page_heian_kitan",
)
...
def exploration_to_six_gates(task) -> bool:
    """探索前往六道之门, 处理不同入口情况"""
    if (
        task.appear_then_click(GameUiAssets.I_EXPLORATION_TO_MOON_SEA)
        or task.appear_then_click(GameUiAssets.I_EXPLORATION_TO_INCENSE_REALM)
        or task.appear_then_click(GameUiAssets.I_EXPLORATION_TO_SEASONRIFT_FOREST)
        or task.appear_then_click(GameUiAssets.I_EXPLORATION_TO_PURE_BUDDHA_REALM)
        or task.appear_then_click(GameUiAssets.I_EXPLORATION_TO_MANTRA_TOWER)
        or task.appear_then_click(GameUiAssets.I_EXPLORATION_TO_PEACOCK_KINGDOM)
    ):
        return True
    return False
...
page_six_gates = Page(
    any_of(
        GameUiAssets.I_CHECK_MOON_SEA,
        GameUiAssets.I_CHECK_INCENSE_REALM,
        GameUiAssets.I_CHECK_SEASONRIFT_FOREST,
        GameUiAssets.I_CHECK_PURE_BUDDHA_REALM,
        GameUiAssets.I_CHECK_MANTRA_TOWER,
        GameUiAssets.I_CHECK_PEACOCK_KINGDOM,
    ),
    category="global",
    priority=25,
)
page_six_gates.connect(
    page_exploration,
    GlobalGameAssets.I_UI_BACK_BLUE,
    key="page_six_gates->page_exploration",
)
page_exploration.connect(
    page_six_gates,
    action=exploration_to_six_gates,
    key="page_exploration->page_six_gates",
)

page_bondling_fairyland = Page(
    GameUiAssets.I_CHECK_BONDLING_FAIRYLAND, category="global"
)
page_bondling_fairyland.connect(
    page_exploration,
    GlobalGameAssets.I_UI_BACK_YELLOW,
    key="page_bondling_fairyland->page_exploration",
)
page_exploration.connect(
...
page_hero_test.connect(
    page_exploration,
    GlobalGameAssets.I_UI_BACK_YELLOW,
    key="page_hero_test->page_exploration",
)
page_exploration.connect(
    page_hero_test,
    GameUiAssets.I_EXPLORATION_GOTO_HERO_TEST,
    key="page_exploration->page_hero_test",
)
...
def handle_battle_page(task) -> bool:
    ...
    from tasks.Component.GeneralBattle.general_battle import (
        run_task_or_default_general_battle,
    )
...
page_battle_team = Page(
    any_of(
        GeneralInviteAssets.I_GI_EMOJI_1,
...
page_battle_team_exit.connect(
    page_battle_team,
    GlobalGameAssets.I_UI_CANCEL,
    key="page_battle_team_exit->page_battle_team",
)
page_battle_team.connect(
    page_battle_team_exit,
    GlobalGameAssets.I_UI_BACK_YELLOW,
    key="page_battle_team->page_battle_team_exit",
)
