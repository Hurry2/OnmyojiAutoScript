...
# 来源：self 分支 1a9a1f61 的 module/atom/scatter.py（首个版本为 a93521f7）。
# 本分支尚未引入 self 上 24010e18「重构(点击规则)：统一图像与OCR点击并支持
# Scatter密度模型」对 RuleClick 的改造，因此这里做了一个最小适配：
#   1. 不再向 RuleClick 传 functional，改为自己保存；
#   2. 自带一份任务点击历史（task_click_history / _record_task_click）。
# 若以后整份移植 24010e18，请直接用上游版本覆盖本文件并删掉这两处适配。
import math
import time
...
class RuleScatter(RuleClick):
    ...
    def __init__(
        ...
        # 本分支的 RuleClick 没有 functional 参数，这里自行保存。
    ...
    # 上游 24010e18 把这套历史放在 RuleClick 上，本分支暂时放在这里。
    _task_clicks: list = []
    _task_click_history_limit = 256

    @classmethod
    def task_click_history(cls) -> tuple:
        return tuple(cls._task_clicks)

    def _record_task_click(self, point: tuple[int, int]) -> None:
        history = RuleScatter._task_clicks
        history.append((point[0], point[1], time.monotonic(), self.functional))
        del history[:-RuleScatter._task_click_history_limit]
